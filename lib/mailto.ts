...
export function adoptionMailto(rescuerEmail: string, animalName: string, animalRef: string): string {
  const subject = `Adoption inquiry: ${animalName} (${animalRef})`;
  const body = [
...
// Fostering is UAE-local — the rescuer needs the animal reachable.
export function fosterMailto(rescuerEmail: string, animalName: string, animalRef: string): string {
  const subject = `Foster inquiry: ${animalName} (${animalRef})`;
  const body = [
