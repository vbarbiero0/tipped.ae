...
Tagline / hero line: **"Help a rescued animal find their home."** Brand tagline for marks/products/footer: **"Every tipped ear is an animal someone saved."** (Both changed with the 2026-07-22 voice pass.)
...
| `/rescuers` | **Designed — match `Tipped Rescuers Page.dc.html` (2026-07-22).** Hero with avatar cluster + totals; photo-topped cards (first pet photo, 64px rounded-square avatar overlapping, name, emirate · pets-saved, blurb, cream chips for socials/clinics, "Their pets & wishlist →" + "Email {First}" outline button); "new rescuer" tip-pink badge for rescuers with no pets yet; dashed invite tile → `#join` (the application form — kept, with the Instagram grid, below the directory); cream trust note on the no-money rule. Implementation deltas from the design file, logged in session 14: live data everywhere, invite CTA routes to the form not hello@tipped.cat, no "vetted" wording, EXAMPLE PROFILE chips kept per Hard rule 5. |
...
| `/blog` | **Blog (2026-07-22; renamed from /advice the same day — 301s in place).** MDX posts in `content/blog/` (frontmatter: title, category, summary, cover, date, author "tipped") — no database table. List page with a category filter (Cats · Dogs · Community care · Adopting & travel; only categories with posts render); `/advice/[slug]` article layout with cover, related-posts row, and a soft closing line inviting readers to meet the pets. Voice per Hard rule 4. Covers are branded SVGs in `public/blog/` (they moved with the rename — the /advice redirect would have swallowed them, the route-shadow landmine again) (brand colors + ear motif, no stock photos); og:image is always the dynamic PNG from `app/blog/[slug]/opengraph-image.tsx` because chat apps don't render SVG og:images. |
...
- **Content (ongoing from 2026-07-22):** the `/blog` launched with 6 UAE-specific care posts; new posts are just `.mdx` files dropped into `content/blog/` + an SVG cover in `public/blog/` — no deploy config, no database.
...
**Blog (2026-07-22, session 14; launched as /advice, renamed to /blog the same day at Vanessa's request — 301s from /advice*).** `/blog` + `/blog/[slug]`, MDX via `next-mdx-remote/rsc` + `gray-matter` (`lib/advice.ts` is the loader; posts sorted by date, related = same category first). Launched with 6 posts written in the warm voice, dated 2026-06-26 → 2026-07-19 so the section doesn't look filled in one day: community cats in summer, heatstroke signs, hot pavement (7-second test), cats under cars, winter shelters, and TNR/the ear-tip. Medical content stays at signs-and-first-steps — the answer to an emergency is always "get to a vet now"; no dosages or protocols, ever. Covers: generated branded SVGs in `public/advice/` — **route-shadow check done**: no redirect touches `/advice`, and static `/advice/*.svg` files serve correctly alongside the dynamic `/advice/[slug]` route (filesystem wins; verified with curl). OG images are per-post PNGs from the dynamic `opengraph-image.tsx` (SVG og:images don't preview in WhatsApp). Nav: dropdown row "Tips & advice" + desktop bar link (lg+) + footer link. The brief referenced an approved reference post for tone; none exists in the repo — the brief's own structural spec (short intro, sub-headed sections, bolded lead-ins, "know the danger signs", italic closing with one gentle action) was used as the reference, and all six posts follow it.

**Instagram grid on /rescuers (2026-07-22, session 14 addendum).** `components/InstagramGrid.tsx` renders the last 9 posts from @tipped.ae in a 3×3 rounded-square grid ("FROM THE COMMUNITY · Rescue life, as it happens"), each linking to the post; refreshed at most hourly (`next.revalidate` on the Graph calls). Powered by the official Instagram API (Instagram Login): the long-lived token lives in `app_settings` (migration 0012 — RLS enabled, zero policies, service-role only) so it can **self-refresh** — any fetch finding it >24h old calls `refresh_access_token` and saves the new token back; tokens last 60 days, so the loop never expires while the site gets traffic. Until Vanessa completes the one-time connect (Meta developer app → Instagram Login → long-lived token, pasted to the agent and inserted into app_settings), `getInstagramPosts` returns null and the section renders a follow card instead — never a broken block.
