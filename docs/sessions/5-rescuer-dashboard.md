...
## Addendum 2 — real rescuer auth (2026-07-20)

First rescuer account is live: auth user **vbarbiero0@gmail.com** (Vanessa's), linked to Silvana's rescuer row for testing. Sign-in is **username `straycatdubai`** + the generated password in `.env.local` (`TIPPED_LOGIN_PASSWORD`) — she should change it in Supabase → Authentication when convenient. Migrations 0002 (username→AUTH email lookup — login email is decoupled from the public contact email) and 0003 (authenticated may read rescuers.auth_user_id for the who-am-I query) applied.

The dashboard is now fully database-backed: login page does RPC + signInWithPassword with error state; /dashboard* routes guard via `useRescuer` (redirect to login, sign-out in the sidebar card); My animals lists the signed-in rescuer's rows, status cycling and deletes write through (optimistic with rollback); the add/edit form inserts/updates with photo uploads to `animal-photos` and certificate uploads to the private `vet-certificates/{auth_uid}/…`; the profile page loads and saves everything including inline-editable clinic + wishlist jsonb lists and avatar upload.

Verified: RPC→email, password grant, RLS who-am-I, rescuer-scoped write + revert, anon write silently filtered (0 rows), and the browser route guard. Field-mapping notes: spayed/neutered + vaccinated checkboxes drive the site's precise booleans; the other nine checklist items land in `medical_checks`; `microchipped` is inferred from the chip number; the form doesn't yet expose for_adoption/for_foster intents (defaults true/false on create, preserved on edit) — a known gap to design later.
